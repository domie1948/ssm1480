...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/userAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("user/queryOne?id="+id);
	        }
        function pass(id){
            frameset_layer().open("request/passUpdate?id="+id);
        }
        function deleteAll(power){
    		...
    		if(window.confirm("您确定删除这些数据吗?")){
    			window.location.href="user/informationDeleteAll?idAll="+idAll;
    		}
    		}
        </script>
<script type="text/javascript">
...
<body>
	...
	<form action="user/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${pager.datas }" var="user" varStatus="vg">
						<tr>
							<td><c:out value="${user['loginName'] }"></c:out></td>
							<td><c:out value="${user['userName'] }"></c:out></td>
							<td><c:out value="${user['phoneNo'] }"></c:out></td>
							<td><c:out value="${user['roleInfo']['title'] }"></c:out></td>
							<td><a href="javascript:;" onclick="update(${user['id']})"
								class="tablelink">查看</a>
								<a href="javascript:;" onclick="pass(${user['id']})"
								class="tablelink">修改密码</a> <a href="javascript:;"
								onclick="deletecheck(${user['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="user/query?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="user/query?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="user/query?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="user/query?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="user/query?pageNow=${count }">${count}</a></li>
