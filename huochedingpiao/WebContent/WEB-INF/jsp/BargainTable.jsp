...
<script type="text/javascript">
        $(function(){
	        ...
	        function update(id){
	            	frameset_layer().open("bargain/informationLoadOne?id="+id);
	        }
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${bargainList.datas }" var="bargain" varStatus="vg">
						<tr>
							<td><c:out value="${bargain['information']['number'] }"></c:out></td>
							<td><c:out value="${bargain['information']['name'] }"></c:out></td>
							<td><c:out value="${bargain['userInfo']['userName'] }"></c:out></td>
							<td><c:out value="${bargain['custom']['name'] }"></c:out></td>
							<td><c:out value="${bargain['information']['start'] }"></c:out></td>
							<td><c:out value="${bargain['information']['end'] }"></c:out></td>
							<td><c:out value="${bargain['information']['start_time'] }"></c:out></td>
							<td><c:out value="${bargain['information']['end_time'] }"></c:out></td>
							<td><c:out value="${bargain['information']['price'] }"></c:out></td>
							<td>
				                <c:if test="${bargain['flag'] eq 0}">
				                		<c:out value="未取票" />
				                </c:if>
				                <c:if test="${bargain['flag'] eq 1}">
				                		<c:out value="已取票" />
				                </c:if>
				                 <c:if test="${bargain['flag'] eq 2}">
				                		<c:out value="已退款" />
				                </c:if>
	                			</td>
							<td><a href="javascript:;"
								onclick="deletecheck(${bargain['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="bargain/informationLoad?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="bargain/informationLoad?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="bargain/informationLoad?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="bargain/informationLoad?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="bargain/informationLoad?pageNow=${count }">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="bargain/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
