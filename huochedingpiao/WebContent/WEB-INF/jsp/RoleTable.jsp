...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/roleAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("role/queryOne?id="+id);
	        }
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${pager.datas }" var="role" varStatus="vg">
						<tr>
							<td><c:out value="${role['title'] }"></c:out></td>
							<td><c:out value="${role['name'] }"></c:out></td>
							<td><c:out value="${role['remark'] }"></c:out></td>
							<td><a href="javascript:;" onclick="update(${role['id']})"
								class="tablelink">查看</a>
								<a href="javascript:;"
								onclick="deletecheck(${role['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="role/queryAll?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="role/queryAll?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="role/queryAll?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="role/queryAll?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="role/queryAll?pageNow=${count }">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="role/roleDelete?id="+id;
        		}
        	})
        }
        $(function() {
