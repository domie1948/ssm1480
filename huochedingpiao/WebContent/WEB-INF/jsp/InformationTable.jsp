...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/informationAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("information/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="information/informationLoad" method="POST"
		onsubmit="return check();">
		<div class="formbody">
			<div id="usual1" class="usual">
				<ul class="seachform1">
					<li><label>模糊搜索</label><input type="text" class="scinput1"
						name="searchName" value="${information['searchName'] }"></li>
					<li><input name="" id="cx" type="submit" value="查询" /></li>
					<li><input name="" type="submit" id="cx1" value="查询"
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${informationList.datas }" var="information" varStatus="vg">
						<tr>
							<td><c:out value="${information['number'] }"></c:out></td>
							<td><c:out value="${information['name'] }"></c:out></td>
							<td><c:out value="${information['start'] }"></c:out></td>
							<td><c:out value="${information['end'] }"></c:out></td>
							<td><c:out value="${information['start_time'] }"></c:out></td>
							<td><c:out value="${information['end_time'] }"></c:out></td>
							<td><c:out value="${information['price'] }"></c:out></td>
							<td><a href="javascript:;" onclick="update(${information['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${information['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="information/informationLoad?pageNow=${pager.pageNow }&searchName=${information['searchName']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="information/informationLoad?pageNow=${pager.pageNow-1 }&searchName=${information['searchName']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="information/informationLoad?pageNow=${vs.count }&searchName=${information['searchName']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="information/informationLoad?pageNow=${count }&searchName=${information['searchName']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="information/informationLoad?pageNow=${count }&searchName=${information['searchName']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="information/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
