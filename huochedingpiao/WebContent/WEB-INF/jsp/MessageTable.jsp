...
<script type="text/javascript">
        $(function(){
	        ...
	        function update(id){
	            	frameset_layer().open("message/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="message/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					<c:forEach items="${messageList.datas }" var="message" varStatus="vg">
						<tr>
							<td><c:out value="${message['userInfo']['userName'] }"></c:out></td>
							<td><c:out value="${message['reply']['userName'] }"></c:out></td>
							<td><c:out value="${message['createtime'] }"></c:out></td>
							<td><a href="javascript:;" onclick="update(${message['id']})"
								class="tablelink">回复</a> <a href="javascript:;"
								onclick="deletecheck(${message['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="message/informationLoad?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="message/informationLoad?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="message/informationLoad?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="message/informationLoad?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="message/informationLoad?pageNow=${count }">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="message/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
