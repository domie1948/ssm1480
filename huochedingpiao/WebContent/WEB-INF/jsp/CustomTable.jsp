...
<body>
	...
	<form action="custom/informationLoad" method="POST"
		onsubmit="return check();">
		<div class="formbody">
			<div id="usual1" class="usual">
				<ul class="seachform1">
					<li><label>乘客姓名</label><input type="text" class="scinput1"
						name="name" value="${custom['name'] }"></li>
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${customList.datas }" var="custom" varStatus="vg">
						<tr>
							<td><c:out value="${custom['name'] }"></c:out></td>
							<td><c:out value="${custom['age'] }"></c:out></td>
							<td><c:out value="${custom['sex'] }"></c:out></td>
							<td><c:out value="${custom['id_card'] }"></c:out></td>
							<td><c:out value="${custom['createtime'] }"></c:out></td>
							<td><a href="javascript:;"
								onclick="deletecheck(${custom['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="custom/informationLoad?pageNow=${pager.pageNow }&name=${custom['name']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="custom/informationLoad?pageNow=${pager.pageNow-1 }&name=${custom['name']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="custom/informationLoad?pageNow=${vs.count }&name=${custom['name']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="custom/informationLoad?pageNow=${count }&name=${custom['name']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="custom/informationLoad?pageNow=${count }&name=${custom['name']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="custom/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
