...
<table width="100%" height="27" border="0" cellpadding="0" cellspacing="0" >
  <tr>
    ...
    <td class="shopcarttitle">您有以下订单</td>
  </tr>
</table>
<br>
<br>
<table width="100%" border="0" cellpadding="3" cellspacing="0" class="shopcart">
		  <tr >
		    <td class="cap">&nbsp;</td> 
			<td class="cap">车次编号</td>
			<td class="cap">车次名称</td>
			<td class="cap">乘客姓名</td>
			<td class="cap">起始地</td>
			<td class="cap">目的地</td>
			<td class="cap">出发时间</td>
			<td class="cap">到达时间</td>
			<td class="cap">车次价格</td>
			<td class="cap">状态</td>
			<td class="cap">操作</td>
		  </tr>
		  <c:forEach items="${pager.datas }" var="bargain" varStatus="vs">
                <tr >
	                <td>&nbsp; </td>
	                <td>${bargain['information']['number'] }</td>
	                <td>${bargain['information']['name'] }</td>
	                <td>${bargain['custom']['name'] }</td>
	                <td>${bargain['information']['start'] }</td>
	                <td>${bargain['information']['end'] }</td>
	                <td>${bargain['information']['start_time'] }</td>
	                <td>${bargain['information']['end_time'] }</td>
	                <td>${bargain['information']['price'] }</td>
	                	<td>
                ...
                <td>
                <c:if test="${bargain['flag'] eq 0}" var="t1">
                <a href="bargain/getTicket?id=${bargain['id'] }&flag=1">取票</a>
                <a href="bargain/backTicket?id=${bargain['id'] }&flag=2">退款</a>
                <a href="bargain/informationLoadOne?id=${bargain['id'] }">修改</a>
                </c:if>
                <c:if test="${!t1 }">
                		无
                </c:if>
